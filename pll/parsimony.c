  ...
  for(model = 0; model < (size_t) pr->numberOfPartitions; model++)
    {
      ...
      if (perSiteScores)
       {
         rax_posix_memalign ((void **) &(pr->partitionData[model]->perSiteParsScores), PLL_BYTE_ALIGNMENT, (size_t)compressedEntriesPadded * PLL_PCF * sizeof (parsimonyNumber));
         for (i = 0; i < (size_t)compressedEntriesPadded * PLL_PCF; ++i)
...
// Diep: adding if before rax_free
void pllInitParsimonyStructures(pllInstance *tr, partitionList *pr, pllBoolean perSiteScores)
{
	int
		i,
	...
	for (i = 0; i < pr->numberOfPartitions; ++ i){
		if(pr->partitionData[i]->parsVect){
			rax_free (pr->partitionData[i]->parsVect);
			pr->partitionData[i]->parsVect = NULL;
		}
	}

	if(tr->parsimonyScore){
		rax_free (tr->parsimonyScore);
...
{
	// reset tr->parsimonyScore
	// reset pr->partitionData[model]->perSiteParsScores[
	int i, model;
	for(model = 0; model < (size_t) pr->numberOfPartitions; model++){
		for (i = 0; i < (size_t)pr->partitionData[model]->parsimonyLength * PLL_PCF; ++i)
			pr->partitionData[model]->perSiteParsScores[i] = 0;
	}
...
/*
 * To Tomas:
 * - the perSiteParsScores needs reset at the beginning of pllEvaluateParsimony
 */
